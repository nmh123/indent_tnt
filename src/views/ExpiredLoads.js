...
class ExpiredLoads extends Component {
constructor(props) {
    super(props);
...
loadDataAPI() {
    ...
    let url = config.APP_URL + "/load/list?" + page + "&status=EXPIRED";
    ...
    url += "&sort_type=" + this.state.sortOrder;
    }
    ...
    if (self.state.filterStartDate != "" && self.state.filterStartDate != "null") {
    ...
    .then(
        ...
        for (let row of response.rows) {
            ...
            let newpick_timeZone = moment(obj.pick_appointment_time).format("MM/DD/YYYY HH:mm ");
...
updatePage(pages) {
    ...
    `/expired-loads/?page_num=${pages + 1}&per_page=${
        this.state.perPages
    }&status=${
...
updatePerPage(v) {
    this.setState({ perPages: v, pages: 0 },()=>{this.update_user_setting( this.props.history.push(
        `/expired-loads/?page_num=${1}&per_page=${v}&status=${
            this.state.filterStatus ? this.state.filterStatus : null
        }&tracking_mode=${
            this.state.filterTrackingMode ? this.state.filterTrackingMode : null
        }&tracker=${
...
filter() {
    ...
    this.props.history.push(
    `/expired-loads/?page_num=${1}&per_page=${
        this.state.perPages
    }&tracking_mode=${
        this.state.filterTrackingMode ? this.state.filterTrackingMode : null
    }&tracker=${
...
resetFilter() {
    ...
    this.props.history.push("/expired-loads/");
}
            ...
            <div className="col-lg-9">
                ...
                <CardHeader className="border-0">
                    ...
                    <div className="col-md-9">
                        <h2 className="mt-4">Expired Loads</h2>
                    </div>
...
export default withRouter(ExpiredLoads);
