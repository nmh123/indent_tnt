...
class LoadsHistory extends Component {
constructor(props) {
    super(props);
...
componentDidMount() {
    ...
    self.setState({
    pages:
        page_num && page_num != null && page_num != 0
        ? parseInt(page_num) - 1
        : 0,
    perPages: per_page && per_page != undefined ? parseInt(per_page) : 20,
    filterTrackingMode:
        tracking_mode && tracking_mode != undefined ? tracking_mode : "",
    filterExternalTracker:
    ...
    console.log("param>>", tracking_mode, priority, risk);
...
async update_user_setting(cb) {
    ...
    body: JSON.stringify({
        column_order: JSON.stringify(self.state.colsSetting),
        sort_order: self.state.sortOrder,
    ...
    .then((response) => {
        ...
        if (cb) {
        self.onSortItems(self.state.cols);
        self.get_user_setting(() => self.loadDataAPI());
        }
    }).catch(err=>{
...
loadDataAPI() {
    ...
    let url = config.APP_URL + "/load/list?" + page + "&status=DELIVERED";
    ...
    .then(
        ...
        for (let row of response.rows) {
            ...
            else if (obj.dest_city_state && obj.dest_city_state == null) 
            {
...
updatePage(pages) {
    let self = this;
    this.setState({ pages }, () => {this.update_user_setting(this.props.history.push(
        `/delivered-loads/?page_num=${pages + 1}&per_page=${
            this.state.perPages
        }&status=${
...
updatePerPage(v) {
    this.setState({ perPages: v, pages: 0 }, () => {
    this.update_user_setting();
    this.loadDataAPI();
    });
    this.props.history.push(
    `/delivered-loads/?page_num=${1}&per_page=${v}&status=${
        this.state.filterStatus ? this.state.filterStatus : null
    }&tracking_mode=${
        this.state.filterTrackingMode ? this.state.filterTrackingMode : null
    }&tracker=${
...
filter() {
    ...
    this.props.history.push(
    `/delivered-loads/?page_num=${1}&per_page=${this.state.perPages}&status=${
        this.state.filterStatus ? this.state.filterStatus : null
    }&tracking_mode=${
        this.state.filterTrackingMode ? this.state.filterTrackingMode : null
    }&tracker=${
...
resetFilter() {
    ...
    this.props.history.push("/delivered-loads/");
}
            ...
            <div className="col-lg-9">
                ...
                <CardHeader className="border-0">
                    ...
                    <div className="col-md-9">
                        <h2 className="mt-4">Delivered Loads</h2>
                    </div>
                ...
                {this.state.loading == true ? (
                    ...
                    <Table
                    id="myTable"
                    className="align-items-center table-flush text-capitalize"
                    responsive
                    >
                    <thead className="thead-light text-capitalize">
                        <tr className="sortable-list text-capitalize">
                        {listItems}
                        </tr>
                    </thead>
...
export default withRouter(LoadsHistory);
